...
QString Application::getResult(const QVector<FileHasher *> &jobs, QString parentDir, bool includeMethod, bool removeDups)
{
    qint64 totalSize = 0;
    ...
    for (int i = 0; i < jobs.size(); ++i)
    {
        ...
        QString method = (includeMethod ? jobs[i]->methodStr() + " " : "");
        delete jobs[i];

        if (includeMethod == false)
        {
            if (name.contains(".git/objects/"))
                continue; // TODO: add option
            if (name.endsWith("/desktop.ini")) // TODO: wtf?!
            {
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        {
            qint64 fileSize = hash2size[hash2count_itr.key()];
            qint64 size_mb = fileSize / (1024 * 1024);
            bool size_ok = (size_mb >= min_size[i] && (i == 0 || size_mb < min_size[i - 1]));
            if (hash2count_itr.value() > 1 && size_ok && fileSize > 0)
            {
                num_duplicates++;
                wasted += (hash2count_itr.value() - 1) * fileSize;
